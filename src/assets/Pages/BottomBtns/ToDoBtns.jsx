...
const BtnsToDo = () => {
    ...
    useEffect(() => {
        if(currentBtn === 0){
            if(page === 'ToDoMain') setBtnState(4);
            else if(page === 'ToDoMetrics') setBtnState(1);
        }
    }, [currentBtn]);
...
function BottomPanel({page,addPanel,theme,currentBtn,setBtnState,notify})
{
    ...
    return (    
        <div style={styles(theme,currentBtn).style}>
          {page !== 'ToDoMain' && addPanel === '' && ( <Back style={styles(theme,currentBtn,-1,false,false).btnstyle} onClick={() => {onBack(page,addPanel);setCurrentBottomBtn(-1);}} />)}
          {addPanel !== '' && ( <Back style={styles(theme,currentBtn,-1,false,false).btnstyle} onClick={() => {onBack(page,addPanel);setCurrentBottomBtn(-1);}} />)}
          {page === 'ToDoMain' && addPanel === '' && ( <Home style={styles(theme,currentBtn,-1,false,false).btnstyle} onClick={() => {onBack(page,addPanel);setCurrentBottomBtn(-1);}} />)}
            <Add style={{...styles(theme,currentBtn,53,true,addPanel === '' && page !== 'ToDoMain').btnstyle}} onClick={() => {if(page === 'ToDoMain'){setCurrentBottomBtn(3);setAddPanel('ToDoNew');playEffects(switchSound);}}} />
          
            <FaChartLine style={styles(theme,currentBtn,1,false,false).btnstyle} onClick={() => {setCurrentBottomBtn(1);setPage('ToDoMetrics');setAddPanel('');playEffects(switchSound);}} />
