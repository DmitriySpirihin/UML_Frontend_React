...
import { FaStarHalf, FaStar, FaInfinity, FaSpa, FaTrophy, FaCrown, FaCalculator } from 'react-icons/fa';
...
const MathMain = () => {
    ...
    const pageInfo = { Icon: FaCalculator, ru: 'Быстрый счет', en: 'Mental Math' };
    const HeroIcon = pageInfo.Icon;
    const s = styles(theme, fSize);
    const totalRecord = (AppData.mentalRecords?.[0] || []).reduce((sum, value) => sum + (Number(value) || 0), 0);
    const bestRecord = Math.max(0, ...(AppData.mentalRecords?.[0] || [0]));
                ...
                <Motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.34 }} style={s.hero}>
                    ...
                    <div style={s.heroTop}>
                        ...
                        <div style={s.heroText}>
                            <div style={s.eyebrow}>{langIndex === 0 ? 'Скорость' : 'Speed'}</div>
                            <h1 style={s.heroTitle}>{langIndex === 0 ? pageInfo.ru : pageInfo.en}</h1>
                            <div style={s.heroSubtitle}>{langIndex === 0 ? 'Тренировка счета, темпа и точности' : 'Train pace, calculation, and accuracy'}</div>
                        </div>
                    </div>
...
function MathCard({ protocol, difficulty, onClick, theme, lang, fSize, variants, needBlur, record, isFullWidth }) {
...
const styles = (theme, fSize = 0) => {
    ...
    return {
        ...
        hero: {
            ...
            boxSizing: 'border-box',
            background: isLight
                ? `linear-gradient(145deg, rgba(255,255,255,0.96), rgba(${accentRgb},0.12))`
                : `linear-gradient(145deg, rgba(23,27,31,0.96), rgba(${accentRgb},0.14))`,
            border: `1px solid ${isLight ? 'rgba(15,23,42,0.08)' : 'rgba(102,217,232,0.28)'}`,
            boxShadow: isLight
                ? `0 16px 38px -34px rgba(${accentRgb},0.45), 0 1px 0 rgba(255,255,255,0.72) inset`
                : `0 18px 40px -34px rgba(${accentRgb},0.50), 0 1px 0 rgba(255,255,255,0.055) inset`,
        ...
        heroGlow: {
            ...
            height: 170,
            borderRadius: '50%',
            background: `radial-gradient(circle, rgba(${accentRgb},0.24) 0%, transparent 62%)`,
            pointerEvents: 'none'
        },
        ...
        heroIcon: {
            ...
            color: accent,
            boxShadow: `0 12px 26px -20px rgba(${accentRgb},0.85), inset 0 1px 0 rgba(255,255,255,0.08)`,
            fontSize: 21,
            flexShrink: 0
        },
        heroText: { minWidth: 0 },
