...
4) 🎯 Strategy: Specific interval training or pedaling technique recommendation for next week. Reference terrain impact from session notes.`]};function Ce(n,m=d.GENERAL){const r=t=>{const s=t.getTimezoneOffset()*6e4;return new Date(t.getTime()-s).toISOString().split("T")[0]},p=new Date,i=[];for(let t=6;t>=0;t--){const s=new Date(p);s.setDate(s.getDate()-t),i.push(r(s))}const f=u.pData||{},E=Q.name?.trim()||(n===0?"Пользователь":"User"),D=u.habitsByDate||{},C=u.trainingLog||{},T=u.breathingLog||{},v=u.meditationLog||{},A=u.hardeningLog||{},F=u.sleepingLog||{};u.mentalLog;const G=u.todoList||[],P=u.programs||{},O=u.exercises||{},H=he||{},a=(t,s)=>s.length===0?`${t} (last 7 days):
  No data found
`:`${t} (last 7 days):
${s.join(`
`)}
...
USER CONTEXT:
- Name: ${E}
- Profile: ${f.age||"?"} y.o, ${f.gender===0?"Male":"Female"}, Goal: ${f.goal!==void 0?["Mass","Strength","Cut","Health"][f.goal]:"General"}
`.trim(),I=G.map(t=>{const s=t.goals||[],e=s.filter(c=>c.isDone).length,h=s.length>0?`(${e}/${s.length} goals)`:"",g=t.isDone?"✅ DONE":"⏳ IN PROGRESS";return`  - [${t.category}] ${t.name}: ${g} ${h} | Priority: ${t.priority}/5, Urgency: ${t.urgency}/5, Difficulty: ${t.difficulty}/5 | Deadline: ${t.deadLine}`}),k=a("TO-DO LIST & PRODUCTIVITY",I),U=[];i.forEach(t=>{const s=F[t];if(!s)return;const e=Math.round((s.duration||0)/36e4)/10;U.push(`  ${t}: Sleep=${e}h, Mood=${s.mood}/5, Note="${s.note||""}"`)});const te=a("SLEEP_AND_RECOVERY",U),W=[];i.forEach(t=>{(Array.isArray(T[t])?T[t]:[]).forEach((e,h)=>{if(!e?.startTime||!e?.endTime)return;const g=Math.round((e.endTime-e.startTime)/6e4),c=e.maxHold?Math.round(e.maxHold/1e3):0;W.push(`  ${t} [Session ${h+1}]: Duration=${g} min, Max Breath Hold=${c} sec`)})});const ne=a("BREATHING_EXERCISES",W),V=[];i.forEach(t=>{(Array.isArray(v[t])?v[t]:[]).forEach((e,h)=>{if(!e?.startTime||!e?.endTime)return;const g=Math.round((e.endTime-e.startTime)/6e4);V.push(`  ${t} [Session ${h+1}]: Duration=${g} min`)})});const oe=a("MEDITATION",V),q=[];i.forEach(t=>{(Array.isArray(A[t])?A[t]:[]).forEach((e,h)=>{if(!e?.startTime||!e?.endTime)return;const g=Math.round((e.endTime-e.startTime)/6e4),c=e.timeInColdWater?Math.round(e.timeInColdWater/6e4):0;q.push(`  ${t} [Session ${h+1}]: Total=${g} min, Cold Exposure=${c} min`)})});const ie=a("HARDENING",q),X=[];i.forEach(t=>{const s=D[t];if(!s)return;const h=(Array.isArray(s)?s:Object.entries(s).map(([g,c])=>({habitId:Number(g),status:c}))).map(g=>{const c=H[g.habitId],$=c?.name?c.name[1]||c.name[0]:`Habit #${g.habitId}`;let S="Skipped";return g.status===-2&&(S="Done"),g.status===1&&(S="Abstained (Success)"),g.status===0&&(S="Failed/Skipped"),`${$}: ${S}`});h.length>0&&X.push(`  ${t}: ${h.join(", ")}`)});const se=a("HABITS",X),L=[];i.forEach(t=>{const s=C[t];!s||!s.length||s.forEach(e=>{const h=e.type&&["RUNNING","CYCLING","SWIMMING"].includes(e.type),g=!h||e.type==="GYM"||!e.type;if(h){let c;e.type==="SWIMMING"?c=`${Math.round(e.distance*1e3)} m`:c=`${e.distance.toFixed(1)} km`;const $=Math.round(e.duration||0);let S="";if(e.type==="RUNNING"&&e.distance>0&&$>0){const x=$/e.distance,w=Math.floor(x),M=Math.round((x-w)*60);S=` | Pace: ${w}:${M.toString().padStart(2,"0")} min/km`}else if(e.type==="CYCLING"&&e.distance>0&&$>0){const x=$/60;S=` | Speed: ${(e.distance/x).toFixed(1)} km/h`}const R=[`Type: ${e.type}`,`Distance: ${c}`,`Duration: ${$} min${S}`];if(e.elevationGain>0&&R.push(`Elevation: ${e.elevationGain} m`),e.avgHeartRate>0&&R.push(`HR: ${e.avgHeartRate} bpm`),e.avgCadence>0){const x=e.type==="CYCLING"?"rpm":"spm";R.push(`Cadence: ${e.avgCadence} ${x}`)}if(e.rpe>0&&R.push(`RPE: ${e.rpe}/10`),e.notes?.trim()){const x=e.notes.trim().length>40?e.notes.trim().substring(0,40)+"...":e.notes.trim();R.push(`Notes: "${x}"`)}L.push(`  DATE: ${t} | ${R.join(" | ")}`)}else if(g){const c=P[e.programId],$=c?.name?Array.isArray(c.name)?c.name[1]||`Prog #${e.programId}`:c.name:`Prog #${e.programId}`,S=Math.round((e.duration||0)/6e4);L.push(`  DATE: ${t} | Program: ${$} | Duration: ${S} min`),(e.exerciseOrder||[]).forEach(x=>{const w=e.exercises?.[x];if(!w)return;const M=O[x],ge=M?.name?Array.isArray(M.name)?M.name[1]||`Ex #${x}`:M.name:`Ex #${x}`;let z=0,Z=0;(w.sets||[]).forEach(Y=>{Y.weight>z&&(z=Y.weight),Z+=Y.reps||0});const ue=w.totalTonnage||0;L.push(`    - ${ge}: Max=${z}kg, Reps=${Z}, Vol=${ue.toFixed(1)}kg`)})}})});const re=a("TRAINING_LOG",L),j=[];i.forEach(t=>{const s=u.mentalLog[t];if(!Array.isArray(s)||s.length===0){j.push(`  ${t}: []`);return}j.push(`  ${t}: [`),s.forEach((e,h)=>{const g=`{type:'${e.type}',difficulty:'${e.difficulty}',duration:${e.duration},scores:${e.scores},rightAnswers:'${e.rightAnswers}',maxPosibleScores:${e.maxPosibleScores}}`;j.push(`    ${g}${h<s.length-1?",":""}`)}),j.push("  ]")});const ae=a("BRAIN_TRAINING",j),ce=(J[n]||J[0]).trim(),le=(K[m][n]||K[m][0]).trim(),de=`
(Task for AI:
1. ALWAYS address the user as "${E}" in your response.
